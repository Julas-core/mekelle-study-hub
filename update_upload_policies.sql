...
CREATE POLICY "Allow all authenticated users to insert materials" ON materials
...
CREATE POLICY "Allow users to update own materials" ON materials
FOR UPDATE TO authenticated
USING (uploaded_by_user_id = auth.uid());

CREATE POLICY "Allow users to delete own materials" ON materials
...
FOR UPDATE TO authenticated
USING (bucket_id = 'course-materials' AND (auth.uid() = (SELECT uploaded_by_user_id FROM materials WHERE file_path = name)));
